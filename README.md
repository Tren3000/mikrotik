...
> [!NOTE]
> **ОБЯЗАТЕЛЬНО ИЗМЕНИТЬ ИМЯ ПОЛЬЗОВАТЕЛЯ В СКРИПТЕ НА СВОЕГО  !!!**
...
+ $\color{green}{BlockWinboxWAN}$ - 3в1: правило в firewall, RAW и script. Создаёт правило в firewall о добавлении ip-адресов, кто ломится на порт 8291(порт winbox), за исключением белого списка. Далее правило в RAW, где идёт блокирока всех ip-адресов address list, до обработки firewall, для снижения нагрузки на роутер. Дополнительно скрипт запускается от имени администратора, по поиску фразы `"denied winbox/dude connect from"` и добавление найденного ip-адреса в address list. Также создаётся правило в планировщике, на ежедневный запуск скрипта в 06:01 утра.

+ $\color{green}{AutoBackUpUSB}$ - скрипт автоматического резервного копирования конфигурации, на USB. Задействуется планировщиком задач ежедневно. Ротация за 30 дней.

+ $\color{green}{Firewall}$ - типовые, но расширенные настройки firewall, с access-листами и защитой входящих пакетов (Fail2ban)
